...
const GRAPH_RANGE = {
    SECU : {low: -1.3, high: 2.3},
    SELE : {low: -1.1, high: 2.0}
}

let calculate_values_components = (values_quest_responses) => {
    let secular = [];
    let self_expression = [];
...
/**
 * @return A pair [SELE, SECU], bound by the current GRAPH_RANGE
 */
let calculate_values_score = (secular, self_expression) => {
    ...
    let secular_not_scaled = _.sum(_.zipWith(secular, SECU_COMP, _.multiply));
    let self_exp_not_scaled = _.sum(_.zipWith(self_expression, SELE_COMP, _.multiply));
    
    return {
        SELE: (self_exp_value <= GRAPH_RANGE.SELE.low) ? GRAPH_RANGE.SELE.low :
            (self_exp_value >= GRAPH_RANGE.SELE.high) ? GRAPH_RANGE.SELE.high : 
            self_exp_value,
        SECU: (secular_value <= GRAPH_RANGE.SECU.low) ? GRAPH_RANGE.SECU.low :
            (secular_value >= GRAPH_RANGE.SECU.high) ? GRAPH_RANGE.SECU.high : 
            secular_value,
     };
}

let test_calculate_values_score = () => {
...
let build_values_map = (element_id, participant_SECU, participant_SELE, countries_data_csv="./data/inglehart-welzel.csv") => {
    const map_elem = document.getElementById(element_id);
    const map_width = map_elem.offsetWidth;
    ...
    //Read the data
    d3.csv(countries_data_csv).then(function (data) {
        // Add X axis
        const x = d3.scaleLinear()
            .domain([GRAPH_RANGE.SELE.low, GRAPH_RANGE.SELE.high])
            .range([0, map_width]);

        // Add Y axis
        const y = d3.scaleLinear()
            .domain([GRAPH_RANGE.SECU.low, GRAPH_RANGE.SECU.high])
            .range([map_height, 0]);
...
export { calculate_values_components, calculate_values_score, build_values_map };
